...
// タイトル: 動的ナビゲーションバー（ハイドレーションエラー対策版）
// 役割: ClientOnlyでUIをラップし、サーバー/クライアント間の表示不整合を完全に防ぐ。
'use client';
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const fetchChildProfile = async () => {
      try {
        const response = await fetch(`/api/children?child_user_id=${user.id}`, { credentials: 'include' });
        if (response.ok) {
          const data = await response.json();
          if (data && data.length > 0) setChildProfileId(data[0].id);
        }
      } catch (error) {
  ...
  const logoutButtonStyle = { padding: '0.5rem 1rem', backgroundColor: 'rgba(255,255,255,0.2)', color: 'white', border: '1px solid rgba(255,255,255,0.3)', borderRadius: '4px', cursor: 'pointer', fontSize: '0.9em', transition: 'background-color 0.2s' };
  ...
  return (
    <nav style={navStyle}>
      ...
      <ClientOnly>
        {isLoading ? (
          <div style={{ fontSize: '0.9em', color: '#666' }}>認証状態確認中...</div>
        ) : isAuthenticated ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <span style={{ padding: '0.5rem', backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: '4px', fontSize: '0.9em' }}>
              👤 {user?.last_name} {user?.first_name} さん ({getRoleDisplayName(user?.role)})
            </span>
            {user?.role === 'admin' && (
